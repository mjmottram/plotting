...
void plotTimeError(RAT::DSReader& dsReader, vector<string> fitNames, vector<string> plotNames, bool clear=true);
...
void plotTimeError(RAT::DSReader& dsReader, vector<string> fitNames, vector<string> plotNames, bool clear)
{
      ...
      for(unsigned int j=0; j<fitNames.size(); j++)
        {
          ...
          if(fitVertex.ContainsTime() && fitVertex.ValidTime())
            // will also test time
            {
              histsT[j]->Fill(fitTime - ( mcTime + 390.0 - ev->GetGTrigTime() ) );
            }
